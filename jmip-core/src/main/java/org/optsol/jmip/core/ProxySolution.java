...
public class ProxySolution<SOLVER, VARCLASS, CONSTANTS extends IConstants>
  ...
  public Object invoke(
    ...
    try {
      if (method.getName().equals("getObjectiveValue")) {
        return model.getObjectiveValue();
      }
      if (method.getName().equals("getSolutionState")) {
        return model.getSolutionState();
      }
      if (method.getName().equals("getBestObjectiveBound")) {
        return model.getBestObjectiveBound();
      }
      if (method.getName().equals("getSolutionTime")) {
      ...
      if (method.getName().startsWith("getDual_")) {
        ...
        IConstraintManager<? super CONSTANTS, VARCLASS, SOLVER> constraintManager =
